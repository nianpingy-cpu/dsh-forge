/**
 * Binary resolution for trivy (ISSUE-017).
 *
 * trivy ships as a standalone Go binary (GitHub releases), not an npm package,
...
/** Resolve the trivy binary from PATH; never falls back to a bare name. */
export function resolveTrivyBinary(): string {
  return (
    resolveInPath("trivy") ??
    missingSentinel(IS_WINDOWS ? "trivy.exe" : "trivy")
  );
}

export const TRIVY_BINARY_HINT =
  "install trivy: see https://trivy.dev/docs/getting-started/installation/ (download from https://github.com/aquasecurity/trivy/releases)";
